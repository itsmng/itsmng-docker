...
docker exec -w /app -u application itsmweb php bin/console itsmng:database:install -H $containerName -u $databaseUser -p $databasePassword -d $databaseName --no-interaction

docker exec -w /app -u application itsmweb rm -rf install/install.php
